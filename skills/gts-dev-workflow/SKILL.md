...
### Step 4（新增）：代码审核 + 出方案等确认

- BDD + 集成测试 + tsc 全通过后执行

1. **审核本次改动所有文件：**
   - 功能是否符合设计预期？改动是否完整实现了需求？
   - 测试是否确实覆盖了改动点？（有没有漏测的路径）
   - 代码质量：契约检查（`requireCheck` / `test()` / `assertTrue()`）是否到位？
   - 防御式编程是否恰当？（参数必传、尽早 throw）
   - 有无附带副作用？（改了不该改的、漏了该改的）
   - 文档/笔记是否同步更新？
...
9. **Step 4 代码审核不能跳过**——必须等兄弟确认后才能提交
